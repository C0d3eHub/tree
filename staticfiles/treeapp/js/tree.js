document.addEventListener("DOMContentLoaded", function () {
  ...
  // ---- ARROWHEAD DEFINITION: Always visible ----
  svg.append("defs").append("marker")
    .attr("id", "arrowhead")
    .attr("viewBox", "0 -6 12 12")
    .attr("refX", 17) // Center of triangle base for markerUnits=userSpaceOnUse
    .attr("refY", 0)
    .attr("markerWidth", 12)
  ...
  function renderTree(data) {
    ...
    g.selectAll(".link")
      ...
      .attr("d", (d) => {
        const rectHeight = 60; // match your node rect height
        const rectHalfHeight = rectHeight / 2;
        const ARROW_SIZE = 12; // Should match markerWidth/viewBox width
        const startX = d.source.x;
        const startY = d.source.y + rectHalfHeight;
        const endX = d.target.x;
        // Make line end inside the arrowhead for a clean look
        const endY = d.target.y - rectHalfHeight + ARROW_SIZE * 0.6;
