document.addEventListener("DOMContentLoaded", function () {
  ...
  function drawNodesAndLinks(root, spine = [], subtree = []) {
    ...
    // Add Info (ℹ️) Button outside the node at the top left
    // Add Info (ℹ️) Button outside the node at the top left with logic for hiding मृत्यु वर्ष if missing
    nodes
      .append("text")
      .attr("class", "info-btn")
      .attr(
  ...
  <div class="modal-title" style="margin-bottom:12px; font-size:1.25rem; color:#2c3e50;">
    ...
    ${window.EDIT_MEMBER_URL && d.data.id ? 
      `<a href="${window.EDIT_MEMBER_URL.replace("{id}", d.data.id)}" 
          id="edit-member-btn-${d.data.id}"
          class="edit-member-btn"
          onclick="window.location.href='${window.EDIT_MEMBER_URL.replace("{id}", d.data.id)}'; return false;"
          style="display:inline-block; margin-left:10px; background:#4caf50; color:white; 
                 ...
                 display:inline-block; line-height:22px; text-align:center;">✏️</span>Edit Member</a>` 
      : ''}
  </div>
  <div style="
      display:flex;
      ...
      background:#f8fafc;
      border-radius:16px;
      box-shadow:0 2px 12px 0 rgba(100,100,160,0.09);
      padding:18px;
  ">
    <img src="${picUrl}" alt="Picture"
      style="
        ...
        margin-bottom:0;
    ">
    <div style="flex:1; min-width:150px; font-size:1.06rem;">
      <div style="
        display:flex; align-items:center; gap:10px; margin-bottom:7px;">
        <span style="font-weight:600; color:#5c6bc0;">नाम:</span>
        <span style="font-weight:500; color:#222;">${d.data.name || "-"}</span>
      </div>
      <div style="
        display:flex; align-items:center; gap:10px; margin-bottom:7px;">
        <span style="font-weight:600; color:#26a69a;">पिता का नाम:</span>
      ...
      <div style="
        display:flex; align-items:center; gap:10px; margin-bottom:7px;">
        <span style="font-weight:600; color:#ffa726;">जन्म वर्ष:</span>
        <span style="font-weight:500; color:#222;">${
          d.data.year_of_birth || "-"
        }</span>
      </div>
      ${
        showDeathYear
      ...
      <div style="
        ...
        <span style="font-weight:600; color:#e57373;">मृत्यु वर्ष:</span>
        <span style="font-weight:500; color:#222;">${d.data.year_of_death}</span>
      </div>`
          : ""
      }
    </div>
...
`;
        modalRoot.innerHTML = `
  <div class="modal-overlay" id="info-modal" style="
    user-select:none;
    padding: 0 0 10px 0;
    border-radius:16px;
    max-width:98vw;
    min-width:230px;
    background: linear-gradient(120deg,#f3e5f5 0%,#e0f7fa 100%);
    box-shadow:0 12px 42px 0 rgba(52,152,219,0.15);
  ">
    ${html}
...
`;
      ...
      <div class="modal-overlay" id="info-modal" style="user-select: none; pointer-events: all;">
        ...
        <div class="modal-action-row">
          <button id="close-info-modal" style="display:inline-block; background:#64b5f6; color:white; padding:6px 22px; border-radius:6px; text-decoration:none; font-weight:bold; border:none; cursor:pointer; font-size:16px;">OK</button>
        </div>
      </div>
    `;
    ...
    try {
      ...
      if (window.USER_TREE_DATA) {
        ...
        if (!window.IS_MY_TREE) {
          // For main tree view, collapse nodes beyond depth 3
        ...
        fetch(url)
          ...
          .then((data) => {
            ...
            if (!window.IS_MY_TREE) {
              // Only collapse for the main tree view
              function collapseDeep(node, depth) {
                if (!node) return;
                if (depth >= 3 && node.children) {
                  node._children = node.children;
